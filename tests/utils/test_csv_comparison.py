    ...
    try:
        ...
        for col in non_numeric_cols:
            if not ref_df_processed[col].equals(result_df_processed[col]):
                ...
                return (
                    False,
                    f"Non-numeric column '{col}' differs at row {first_diff_idx}: ref='{ref_df_processed[col][first_diff_idx]}', result='{result_df_processed[col][first_diff_idx]}'",
                )
...
def compare_csv_files_exact(
    result_csv_path: str, ref_csv_path: str, separator: str = ";"
) -> tuple[bool, str]:
    """
    ...
    if result_lines == ref_lines:
        ...
        for i in range(max_len):
            if i >= len(ref_lines):
                return (
                    False,
                    f"Result file has more lines. First extra line at {i+1}: {result_lines[i]}",
                )
            if i >= len(result_lines):
                return (
                    False,
                    f"Reference file has more lines. First extra line at {i+1}: {ref_lines[i]}",
                )
            if result_lines[i] != ref_lines[i]:
                return (
                    False,
                    f"First difference at line {i+1}: ref='{ref_lines[i]}', result='{result_lines[i]}'",
                )
